...
```bash
CLEARVOIANCE_VERSION=0.0.0-dev         # image tag applied to engine + ui builds
CLEARVOIANCE_API_URL=http://localhost:9101   # baked into the dashboard at BUILD time
```

`CLEARVOIANCE_API_URL` is passed to the dashboard as a build-arg, not a
runtime env. Next.js inlines `NEXT_PUBLIC_*` at build, so changing the
engine URL requires rerunning `docker compose build ui`.
...
The compose file ships with Traefik labels commented on both engine and
dashboard services. To expose over TLS:

### 1. Uncomment the labels

In `deploy/docker-compose.yml`, uncomment the `networks:` and `labels:`
blocks on `engine:` and `ui:`, plus the top-level `networks:` block.

### 2. Set Traefik vars in `.env`
...
### 3. Join or create the external network

```bash
docker network create traefik_proxy   # or whatever TRAEFIK_NETWORK is set to
```

If you already run Traefik elsewhere on this host, reuse that network
name instead — the compose file will attach both services to it.

### 4. Rebuild the dashboard with the public URL

`CLEARVOIANCE_API_URL` bakes into the JS bundle:

```bash
# in deploy/.env:
CLEARVOIANCE_API_URL=https://api.clearvoiance.example.com

docker compose --env-file deploy/.env -f deploy/docker-compose.yml build ui
