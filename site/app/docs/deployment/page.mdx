...
Whoever fills in the form first becomes the admin. In a standard
self-host that's you, but **don't publish the URL before you've
claimed the admin account**. If the deploy goes live and the page
sits there for days, anyone who finds it could take it over. The
usual practice:

1. Deploy behind a private hostname or a loopback tunnel
2. Hit `/setup` yourself
3. Expose the URL publicly
...
Only needed when the dashboard and engine are on different subdomains
(e.g. behind Traefik). On loopback everything is same-origin — leave
these empty.
...
The cookie is HttpOnly + SameSite=Lax + Secure (auto when the request
is HTTPS). CORS is set up to allow credentials only from the listed
origins — leaving the allow-list empty disables CORS entirely, which
is the correct default for same-origin / SDK-only deploys.
