...
docker-compose stack that brings all three up alongside the engine
binary:
...
The engine's gRPC endpoint is now at `127.0.0.1:9100` and the REST
control plane + WebSocket hub at `127.0.0.1:9101`. Visit
`http://127.0.0.1:9101/docs` to see the Swagger UI.
...
Node 24+ required. Every framework integration is an optional peer
...
Open the dashboard at `http://127.0.0.1:3100` (part of the
docker-compose stack). Log in with any non-empty API key — dev-open
mode accepts anything until you provision a real one. You'll see the
active session with its event count ticking up.
